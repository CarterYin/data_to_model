...
def process_file_to_output(input_file, output_file):
    ...
    # 创建备份文件
    backup_file = input_file + '.original_backup'
...
def main():
    """
    主函数：处理original.tsv文件，输出preprocessing.tsv
    """
    print("🚀 数据预处理脚本启动")
    print("功能：1) 添加samples列名 → 2) YES转换为1 → 3) NO转换为0")
    print("输入文件：original.tsv")
    print("输出文件：preprocessing.tsv")
    
    input_file = 'original.tsv'
    output_file = 'preprocessing.tsv'
