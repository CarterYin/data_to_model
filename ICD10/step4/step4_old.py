...
# -*- coding: utf-8 -*-
"""
删除id_ethnic_group_id不为1的样本
对于指定的id_ethnic_group_id字段，如果值不为1，则删除该样本个体
"""
...
def remove_non_ethnic_group_1_samples(input_file, output_file):
    """
    删除id_ethnic_group_id不为1的样本
    ...
    # 统计要删除的样本数量
    # 保留id_ethnic_group_id为1的样本
    samples_to_keep_mask = (df[ethnic_field] == 1)
    samples_to_remove = (~samples_to_keep_mask).sum()
    total_samples = len(df)
    
    print(f"\n筛选结果:")
    print(f"总样本数: {total_samples}")
    print(f"要删除的样本数（id_ethnic_group_id不为1）: {samples_to_remove}")
    print(f"保留的样本数（id_ethnic_group_id为1）: {samples_to_keep_mask.sum()}")
    print(f"删除比例: {samples_to_remove/total_samples*100:.2f}%")
    
    # 显示具体删除的原因
    print(f"\n删除原因统计:")
    na_count = df[ethnic_field].isna().sum()
    not_1_count = ((df[ethnic_field] != 1) & (~df[ethnic_field].isna())).sum()
    print(f"NA值样本: {na_count} 个")
    print(f"非1值样本: {not_1_count} 个")
    
    # 执行筛选：保留id_ethnic_group_id为1的样本
    ...
    # 生成筛选报告
    report_file = output_file.replace('.tsv', '_ethnic_filter_report.txt')
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("id_ethnic_group_id筛选报告\n")
        f.write("=" * 50 + "\n\n")
        f.write(f"输入文件: {input_file}\n")
        f.write(f"输出文件: {output_file}\n")
        f.write(f"筛选时间: {pd.Timestamp.now()}\n\n")
        f.write(f"筛选条件: 保留 {ethnic_field} == 1 的样本\n\n")
        ...
        f.write("删除原因统计:\n")
        f.write(f"NA值样本: {na_count} 个\n")
        f.write(f"非1值样本: {not_1_count} 个\n\n")
...
def main():
    ...
    print("开始执行id_ethnic_group_id筛选...")
    print("=" * 50)
    
    # 执行筛选
    remove_non_ethnic_group_1_samples(input_file, output_file)
