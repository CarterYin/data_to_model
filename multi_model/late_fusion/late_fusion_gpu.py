...
def prepare_modal_data(modal: str):
    """准备某个模态的数据：对齐、标准化并返回矩阵、缩放器与样本索引。"""
    ...
    train_age = train_age.reindex(train_features.index)
    test_age = test_age.reindex(test_features.index)
    y_train = train_age[age_column].to_numpy(dtype=float) / 100.0
    y_test = test_age[age_column].to_numpy(dtype=float) / 100.0

    # 记录样本顺序
    train_index = train_features.index.to_numpy()
    test_index = test_features.index.to_numpy()

    scaler = StandardScaler()
    X_train = scaler.fit_transform(train_features)
    X_test = scaler.transform(test_features)

    return X_train, X_test, y_train, y_test, scaler, train_index, test_index
...
def train_meta_learner_ridge(oof_pred_dict: Dict[str, np.ndarray], y_train: np.ndarray):
    """基于OOF预测训练RidgeCV元学习器，返回已训练模型与系数。"""
    modal_names = list(oof_pred_dict.keys())
    X_oof = np.vstack([oof_pred_dict[m] for m in modal_names]).T
    ridge = RidgeCV(alphas=(0.1, 1.0, 10.0), fit_intercept=False)
    ridge.fit(X_oof, y_train)
    return ridge, modal_names


def compute_soft_voting_weights(modal_best: Dict[str, Dict], strategy: str = 'uniform') -> Dict[str, float]:
    """基于Soft Voting生成融合权重。

    参数:
        modal_best: 每个模态的最优模型信息字典，包含 'best_cv_score' (MAE)
        strategy: 'uniform' | 'inverse_mae'
    返回:
        {modal: weight}
    """
    modals = list(modal_best.keys())
    if strategy == 'uniform':
        w = 1.0 / len(modals) if len(modals) > 0 else 0.0
        return {m: float(w) for m in modals}

    if strategy == 'inverse_mae':
        maes = np.array([float(modal_best[m]['best_cv_score']) for m in modals], dtype=float)
        # 避免除0
        maes = np.where(maes <= 1e-12, 1e-12, maes)
        inv = 1.0 / maes
        s = float(np.sum(inv))
        if s <= 0:
            w = 1.0 / len(modals) if len(modals) > 0 else 0.0
            return {m: float(w) for m in modals}
        weights = inv / s
        return {m: float(w) for m, w in zip(modals, weights)}

    raise ValueError(f"未知Soft Voting策略: {strategy}")
...
def main():
    parser = argparse.ArgumentParser(description='多模态模型级晚期融合（决策树家族，GPU加速）')
    parser.add_argument('--fusion', type=str, default='soft', choices=['soft', 'learned', 'stacking'],
                        help='融合方式: soft=Soft Voting，learned=基于OOF学习非负权重，stacking=RidgeCV元学习器')
    parser.add_argument('--soft-strategy', type=str, default='uniform', choices=['uniform', 'inverse_mae'],
                        help='Soft Voting权重策略: uniform=均匀加权, inverse_mae=按CV-MAE倒数加权')
    args = parser.parse_args()

    print("开始多模态模型级晚期融合（决策树家族，GPU加速）...")
    print(f"融合方式: {args.fusion}  | Soft策略: {args.soft_strategy if args.fusion=='soft' else 'N/A'}")
    ...
    for m in modals:
        print(f"\n准备模态 {m} 数据...")
        X_train, X_test, y_train, y_test, _, train_index, test_index = prepare_modal_data(m)
        modal_data[m] = {
    ...
    # 跨模态统一训练/测试的共同样本顺序
    train_indices_list = [set(modal_data[m]['train_index']) for m in modals]
    test_indices_list = [set(modal_data[m]['test_index']) for m in modals]
    common_train = sorted(list(set.intersection(*train_indices_list)))
    common_test = sorted(list(set.intersection(*test_indices_list)))

    def reindex_matrix(X, original_index, target_index):
        index_to_pos = {idx: i for i, idx in enumerate(original_index)}
        rows = [index_to_pos[i] for i in target_index]
        return X[rows]
    def reindex_vector(y, original_index, target_index):
        index_to_pos = {idx: i for i, idx in enumerate(original_index)}
        rows = [index_to_pos[i] for i in target_index]
        return y[rows]

    for m in modals:
        modal_data[m]['X_train'] = reindex_matrix(modal_data[m]['X_train'], modal_data[m]['train_index'], common_train)
        modal_data[m]['y_train'] = reindex_vector(modal_data[m]['y_train'], modal_data[m]['train_index'], common_train)
        modal_data[m]['X_test'] = reindex_matrix(modal_data[m]['X_test'], modal_data[m]['test_index'], common_test)
        modal_data[m]['y_test'] = reindex_vector(modal_data[m]['y_test'], modal_data[m]['test_index'], common_test)
        modal_data[m]['train_index'] = np.array(common_train)
        modal_data[m]['test_index'] = np.array(common_test)
    ...
    elif args.fusion == 'stacking':
        print("\n训练Stacking元学习器（RidgeCV，基于训练集OOF预测）...")
        oof_preds = {}
        for m in modals:
            model = modal_best[m]['model']
            preds = get_oof_predictions(model, modal_data[m]['X_train'], y_train_global, n_splits=5)
            oof_preds[m] = preds
        ridge, meta_modal_order = train_meta_learner_ridge(oof_preds, y_train_global)
        stacking_coefs = {m: float(c) for m, c in zip(meta_modal_order, ridge.coef_.astype(float))}
        fusion_weights = stacking_coefs        
    else:
        print("\n使用Soft Voting生成融合权重...")
        fusion_weights = compute_soft_voting_weights(modal_best, strategy=args.soft_strategy)
    ...
    # 加权融合或Stacking
    if args.fusion in ('soft', 'learned'):
    ...
    save_late_fusion_report(
        ...
        fusion_method=(
            f"Soft Voting ({args.soft_strategy})" if args.fusion == 'soft' else (
                'Learned Weights (Non-negative LR)' if args.fusion == 'learned' else 'Stacking (RidgeCV)'
            )
        )
    )
