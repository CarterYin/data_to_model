...
def load_data():
    """加载训练和测试数据"""
    # 加载特征数据
    train_features = pd.read_csv('../train_merged.tsv', sep='\t')
    test_features = pd.read_csv('../test_merged.tsv', sep='\t')
...
def prepare_data(train_features, test_features, train_age, test_age):
    ...
    # 处理三个新字段：is_female_baseline, region_code_baseline, occupation3
    # 对region_code_baseline和occupation3进行独热编码
    categorical_columns = ['region_code_baseline', 'occupation3']
    numerical_columns = [col for col in common_features if col not in categorical_columns + ['is_female_baseline']]
    ...
    if categorical_columns:
        ...
        # 训练独热编码器
        categorical_data_train = train_features[categorical_columns].fillna('missing')
        categorical_data_test = test_features[categorical_columns].fillna('missing')
        
        onehot_encoded_train = onehot_encoder.fit_transform(categorical_data_train)
        onehot_encoded_test = onehot_encoder.transform(categorical_data_test)
        
        # 创建独热编码后的列名
        feature_names = []
        for i, col in enumerate(categorical_columns):
            if col in onehot_encoder.categories_[i]:
                feature_names.extend([f"{col}_{cat}" for cat in onehot_encoder.categories_[i]])
            else:
                feature_names.extend([f"{col}_{cat}" for cat in onehot_encoder.categories_[i]])
        
        # 转换为DataFrame
        onehot_df_train = pd.DataFrame(onehot_encoded_train, 
                                      columns=feature_names, 
                                      index=train_features.index)
        onehot_df_test = pd.DataFrame(onehot_encoded_test, 
                                     columns=feature_names, 
                                     index=test_features.index)
        
        # 合并独热编码后的特征
        train_features_processed = pd.concat([
            train_features[numerical_columns + ['is_female_baseline']],
            onehot_df_train
        ], axis=1)
        
        test_features_processed = pd.concat([
            test_features[numerical_columns + ['is_female_baseline']],
            onehot_df_test
        ], axis=1)
    else:
        # 如果没有分类变量，直接使用原始特征
        train_features_processed = train_features[numerical_columns + ['is_female_baseline']]
        test_features_processed = test_features[numerical_columns + ['is_female_baseline']]
    
    # 对数值特征进行标准化（不包括is_female_baseline和独热编码后的特征）
    scaler = StandardScaler()
    
    # 只对原始数值特征进行标准化
    numerical_features_train = train_features_processed[numerical_columns]
    numerical_features_test = test_features_processed[numerical_columns]
    
    X_train_numerical = scaler.fit_transform(numerical_features_train)
    X_test_numerical = scaler.transform(numerical_features_test)
    
    # 将标准化后的数值特征转换回DataFrame
    X_train_numerical_df = pd.DataFrame(X_train_numerical, 
                                       columns=numerical_columns, 
                                       index=train_features_processed.index)
    X_test_numerical_df = pd.DataFrame(X_test_numerical, 
                                      columns=numerical_columns, 
                                      index=test_features_processed.index)
    
    # 合并标准化后的数值特征、is_female_baseline和独热编码后的特征
    if categorical_columns:
        X_train = pd.concat([
            X_train_numerical_df,
            train_features_processed[['is_female_baseline']],
            onehot_df_train
        ], axis=1)
        
        X_test = pd.concat([
            X_test_numerical_df,
            test_features_processed[['is_female_baseline']],
            onehot_df_test
        ], axis=1)
    else:
        X_train = pd.concat([
    ...
    print(f"特征处理完成:")
    print(f"  原始数值特征数量: {len(numerical_columns)}")
    print(f"  独热编码后特征数量: {X_train.shape[1] - len(numerical_columns) - 1}")
...
def save_results(trained_models, test_results, best_model_name):
    ...
    with open('hyperparameter_tuning_gpu_report.txt', 'w', encoding='utf-8') as f:
        ...
        f.write("-" * 20 + "\n")
        f.write("新增字段处理:\n")
        f.write("- is_female_baseline: 性别字段，不进行标准化\n")
        f.write("- region_code_baseline: 地区编码，进行独热编码处理\n")
        f.write("- occupation3: 职业编码，进行独热编码处理\n")
        f.write("标准化策略: 仅对原始数值特征进行标准化，分类特征和性别特征保持原值\n\n")
