...
def prepare_data(train_features, test_features, train_age, test_age):
    ...
    # 处理性别字段：is_female_baseline
    # 只保留数值特征和性别特征
    numerical_columns = [col for col in common_features if col != 'is_female_baseline']
    
    # 对数值特征进行标准化（不包括is_female_baseline）
    scaler = StandardScaler()
    
    # 只对原始数值特征进行标准化
    numerical_features_train = train_features[numerical_columns]
    numerical_features_test = test_features[numerical_columns]
    ...
    # 合并标准化后的数值特征和is_female_baseline
    X_train = pd.concat([
        X_train_numerical_df,
        train_features[['is_female_baseline']]
    ], axis=1)
    
    X_test = pd.concat([
        X_test_numerical_df,
        test_features[['is_female_baseline']]
    ], axis=1)
    ...
    print(f"特征处理完成:")
    print(f"  数值特征数量: {len(numerical_columns)}")
    print(f"  性别特征数量: 1")
...
def save_results(trained_models, test_results, best_model_name):
    ...
    with open('hyperparameter_tuning_gpu_report.txt', 'w', encoding='utf-8') as f:
        ...
        f.write("标准化策略: 仅对数值特征进行标准化，性别特征保持原值\n\n")
